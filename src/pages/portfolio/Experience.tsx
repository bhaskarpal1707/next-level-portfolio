              ...
              <Reveal key={exp.role + exp.period} delay={i * 0.08}>
                <div className="relative pl-12 sm:pl-16">
                  ...
                  <motion.span
                    ...
                    className="absolute left-0 top-1.5 grid h-8 w-8 place-items-center rounded-full border border-teal-300/50 bg-[#0a0d16] sm:h-10 sm:w-10"
                  >
                  ...
                  <div className="glass glass-hover noise relative overflow-hidden rounded-2xl p-6 sm:p-8">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="font-display text-xl font-semibold text-slate-100 sm:text-2xl">
                          {exp.role}
                        </h3>
                        <a
                          href={exp.orgUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="mt-2 inline-flex items-center gap-2 text-sm text-teal-300/90 transition-colors hover:text-teal-200"
                        >
                          <Building2 className="h-3.5 w-3.5" />
                          {exp.org}
                        </a>
                      </div>
                      <span className="glass rounded-full px-4 py-1.5 font-mono2 text-[11px] tracking-[0.18em] text-teal-200/90">
                        {exp.period}
                      </span>
                    </div>
                    ...
                    <ul className="mt-6 space-y-3">
                      {exp.points.map((pt, j) => (
                        <li key={j} className="flex gap-3 text-sm leading-relaxed text-slate-400">
                          <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-teal-300/70" />
                          {pt}
                        </li>
                      ))}
                    </ul>
